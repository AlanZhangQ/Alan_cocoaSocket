...
+ (instancetype)shareInstance
{
    ...
    static ImagePickerManager *pickManager;
    dispatch_once(&once, ^{
        pickManager = [[ImagePickerManager alloc] init];
    });
    return pickManager;
}
...
//        [Tool showLoadingOnWindow];
        ...
        //上传的视频格式转化
        [self movFileTransformToMP4WithSourceUrl:url completion:^(NSString *Mp4FilePath,NSString *uniqueName,NSString *duration) {
...
//            [_moviePlayer play];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            taskModel.duration      =   [self getNewTimeFromDurationSecond:duration.integerValue];
            taskModel.videoPath     =   urlStr;
...
#pragma 视频转化
- (void)movFileTransformToMP4WithSourceUrl:(NSURL *)sourceUrl completion:(void(^)(NSString *Mp4FilePath,NSString *uniqueName,NSString *duration))comepleteBlock
{
    /**
     *  mov格式转mp4格式
     */
    AVURLAsset *avAsset = [AVURLAsset URLAssetWithURL:sourceUrl options:nil];
    float cstTime = CMTimeGetSeconds(avAsset.duration);
    NSString *duration = [NSString stringWithFormat:@"%f",cstTime];
             ...
             switch (exportSession.status) {
                 ...
                 case AVAssetExportSessionStatusCompleted:
                 {
                  ...
                  NSLog(@"AVAssetExportSessionStatusCompleted");
                     ...
                     comepleteBlock(resultPath,uniqueName,duration);
